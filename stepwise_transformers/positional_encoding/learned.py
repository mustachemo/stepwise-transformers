...
class LearnedPositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, positions: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if positions is None:
            ...
            if positions.shape != (batch_size, seq_len):
                raise ValueError(
                    f"Positions shape {positions.shape} doesn't match input shape ({batch_size}, {seq_len})"
                )
    ...
    def compute_embedding_statistics(self) -> dict[str, float]:
        ...
        if self.max_seq_length > 1:
            consecutive_distances = torch.norm(embeddings[1:] - embeddings[:-1], dim=1)
    ...
    def extend_max_length(self, new_max_length: int) -> None:
        ...
        with torch.no_grad():
            ...
            nn.init.normal_(
                new_embeddings.weight[self.max_seq_length :], mean=0.0, std=self.init_std
            )

...
class ClearMLTracker:
    ...
    def log_metrics(
        ...
        for metric_name, value in metrics.items():
            ...
            # Clean metric name
            clean_name = (
                metric_name.replace("train_", "")
                .replace("val_", "")
                .replace("test_", "")
            )

            self.logger.report_scalar(
                title=series, series=clean_name, value=value, iteration=step
            )
    ...
    def log_model_architecture(
        self, model: torch.nn.Module, input_shape: Tuple[int, ...]
    ) -> None:
        """
    ...
    def log_attention_heatmap(
        ...
        if tokens is not None:
            sns.heatmap(
                attn, xticklabels=tokens, yticklabels=tokens, cmap="Blues", cbar=True
            )
    ...
    def log_training_curves(
        ...
        # Log to ClearML
        self.logger.report_matplotlib_figure(
            title="training_progress", series="curves", figure=plt.gcf(), iteration=0
        )
    ...
    def log_gradient_norms(
        self, model: torch.nn.Module, step: Optional[int] = None
    ) -> None:
        """
        ...
        self.logger.report_scalar(
            title="gradients", series="total_norm", value=total_norm, iteration=step
        )

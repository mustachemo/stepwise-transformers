...
from utils.data_utils import (
    SimpleTokenizer, TokenizerConfig, TextPairDataset,
    create_sample_translation_data, create_dataloader
)
...
def calculate_accuracy(predictions: torch.Tensor, targets: torch.Tensor, pad_token_id: int) -> float:
    """
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        if global_step % config['logging']['eval_every'] == 0 and global_step > 0:
            with torch.no_grad():
                ...
                encoder_attn = attention_weights['encoder'][f'layer_{config["model"]["n_layers"]-1}']
...
def main():
    ...
    parser.add_argument("--project-name", type=str, default="stepwise-transformers", 
                       help="ClearML project name")
    parser.add_argument("--task-name", type=str, default="transformer_training", 
                       help="ClearML task name")
    parser.add_argument("--tags", nargs="+", default=["transformer", "training"], 
                       help="ClearML tags")
    ...
    source_texts, target_texts = create_sample_translation_data(config['data']['num_samples'])
    ...
    tracker.log_model_architecture(model, (config['training']['batch_size'], config['model']['max_seq_len']))
    ...
    for epoch in range(config['training']['num_epochs']):
        ...
        val_loss, val_acc = evaluate_model(model, val_loader, criterion, device, "validation")
